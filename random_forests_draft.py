...
forests = RandomForest(X, Y, N_tree, Pred_index[1], Pred_index[0])
Pred = forests.predict(X)
print(Pred)
print(Pred.shape)
MSE = mean_squared_error(Y, Pred, squared=True)
print(MSE)
RMSE = np.sqrt(MSE)
print(RMSE)
...
plt.suptitle('Random Forests, N=' + str(Pred_index[0]) + ', N_tree=' + str(N_tree) + ', depth=' + str(3) + ', min_leaf=' + str(5))
plt.plot(np.arange(len(Y))+1, Y, color="blue", linewidth=2.5, linestyle="-")
plt.plot(np.arange(len(Pred))+1, Pred, color="red",  linewidth=2.5, linestyle="-")
plt.show()
"""
...
plt.plot(np.arange(len(Pred[Pred_index[0]-500:Pred_index[0]]))+1, Pred[Pred_index[0]-500:Pred_index[0]], color="red",  linewidth=2.5, linestyle="-", label='Predicted Price')
plt.xlabel('Hours')
